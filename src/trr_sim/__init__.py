...
def load_track(client):
    ...
    return client.loadURDF(str(TRACK), [0, 0, 0])


def get_waypoints(position: npt.ArrayLike = [0, 0], yaw: float = 0.0):
    """Return the waypoints representing the track

    Passing option position and yaw will modify the waypoints so that the provided position becomes the origin of the referential.
    """
